...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
	    ...
	    cout << execfile[execfileNum] << "Priority: "<< threadPriority[execfileNum] << "\n";
	}
    }
}
...
void Kernel::ExecAll()
{
	//cout << "Kernel:ExecAll()"<<endl;
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i],threadPriority[i]); //MP3
	}
...
{
	//cout << "Kernel::Exec()"<<endl;
	t[threadNum] = new Thread(name, threadNum);
	//MP3
	
	t[threadNum]->setBurstTime(0);
	t[threadNum]->setRunTime(0);
	t[threadNum]->setStartTime(kernel->stats->totalTicks);
