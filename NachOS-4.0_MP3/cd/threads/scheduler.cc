...
//MP3
    ...
    if(priority >= 100 && priority < 150){
        DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() << "] is inserted into queue L[1]"<< endl);
        readyList1->Insert(thread);
    }
    else if(priority >= 50 && priority < 100){
        DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() << "] is inserted into queue L[2]"<< endl);
        readyList2->Insert(thread);
    }
    else {
        DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() << "] is inserted into queue L[3]"<< endl);
...
//    cout << "Tick [" << kernel->stats->totalTicks << "]: In FindNextToRun " << endl;
    ...
    if (!readyList1->IsEmpty()) {
        ...
        //thread->reset_age();
	DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() << "] is removed from queue L[1]" << endl);
	return thread;
   	//return readyList1->RemoveFront();
        ...
        //thread->reset_age();
        DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() << "] is removed from queue L[2]" << endl);
        return thread;
	//return readyList2->RemoveFront();
        ...
        //thread->reset_age();
        DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() << "] is removed from queue L[3]" << endl);
        return thread;
...
Scheduler::Run (Thread *nextThread, bool finishing)
{
    ...
    //MP3
    DEBUG(dbgCPU,"Tick [" << kernel->stats->totalTicks <<"]: Thread [" << nextThread->getID() << "] is now selected for execution" << ", thread [" << oldThread->getID() << "] is replaced, and it has executed [" << kernel->stats->totalTicks - oldThread->getStartTime()<< "] ticks" <<endl );
...
Scheduler::N_Aging(){
	...
	if(t->getPriority() >= 100 ){
            DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << t->getID() << "] is removed from queue L[2]" << endl);
            DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << t->getID() << "] is inserted into queue L[1]" << endl);
	    new_readyList1->Insert(t);
	}
	else{
    ...
    while(!readyList3->IsEmpty()){
        ...
        t->aging();
        if(t->getPriority() >= 50 ){
            DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << t->getID() << "] is removed from queue L[3]" << endl);
            DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << t->getID() << "] is inserted into queue L[2]" << endl);
            new_readyList2->Insert(t);
        }
        else{
