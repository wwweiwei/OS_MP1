...
//MP3
void 
Thread::updateCPUBurst(){
    CPUBurst += kernel->stats->totalTicks - getStartTime();
}
...
//cout<<"aging"<<endl;    
age++;
    //cout << "Thread [" << getID() << "] Age = " << age << endl;
    if(age >= 15){
    	age = 0;
	int oldPriority = getPriority();
	setPriority(oldPriority + 10 );
 	DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << getID() << "] changes its priority from [" << oldPriority << "] to [" << getPriority() << "]" << endl);
    }
...
Thread::Yield ()
{
    //cout << "Thread::Yield()" << endl;
    Thread *nextThread;
    IntStatus oldLevel = kernel->interrupt->SetLevel(IntOff);
    ASSERT(this == kernel->currentThread);
    DEBUG(dbgThread, "Yielding thread: " << name);
 
    kernel->scheduler->ReadyToRun(this);//MP3

    nextThread = kernel->scheduler->FindNextToRun();

//MP3
    //kernel->scheduler->ReadyToRun(this);
   // int approximate = 0.5 * (this->getRunTime() + this->getBurstTime());
   // int lastBurst = this->getBurstTime();
   // this->setBurstTime(approximate);
//   DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID() << "] is now selected for execution" << ", thread [" << this->getID() << "] is replaced, and it has executed [" << this->getRunTime() << "] ticks" << endl);
   // cout << "Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from:[" 
//<< lastBurst << "], add [" << this->getRunTime() << "], to [" <<  this->getBurstTime() << "]" << endl;
...
//MP3
    ...
    this->updateCPUBurst();
    int lastBurst = this->getApproximated_CPUBurst();
    this->updateApproximated_CPUBurst();

    /*
    this->setRunTime(kernel->stats->totalTicks - this->getStartTime());
    int approximate = 0.5 * (this->getRunTime() +this->getBurstTime());
    //int lastBurst = this->getBurstTime();
    this->setBurstTime(approximate);
    */
    //DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] put to sleep and it has executed [" << this->getCPUBurst() << "] ticks, Waiting Time =" << this->getWaitingTime() << endl);
    DEBUG(dbgCPU, "Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from :[" << lastBurst << "], add [" << this->getCPUBurst() << "], to [" << this->getApproximated_CPUBurst() << "]" << endl); 
