...
public:
	...
	void setWaitingTime (int t){ waitTime = t;}
	int getWaitingTime (){return waitTime;}
	...
	int getPriority(){return exePriority;}
	int getApproximated_CPUBurst() { return approximated_CPUBurst;}
	int getCPUBurst () { return CPUBurst;}
	void updateCPUBurst();
	//	CPUBurst += (kernel->stats->totalTicks - getStartTime());}
	void resetCPUBurst () {CPUBurst = 0;}
	void updateApproximated_CPUBurst(){
		if(CPUBurst != 0) approximated_CPUBurst = 0.5 * CPUBurst + 0.5 * approximated_CPUBurst;}
	void setAgingTime(int t){ agingTime = t; }
