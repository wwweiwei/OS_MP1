...
age++;
	...
	setPriority(oldPriority + 10 );
 	DEBUG(dbgCPU, "[C] Tick [" << kernel->stats->totalTicks << "]: Thread [" << getID() << "] changes its priority from [" << oldPriority << "] to [" << getPriority() << "]" );
    }
...
//MP3
    ...
    this->updateCPUBurst();
    double lastBurst = this->getApproximated_CPUBurst();
    ...
    */
    ...
    DEBUG(dbgCPU, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from :[" << lastBurst << "], add [" << this->getCPUBurst() << "], to [" << this->getApproximated_CPUBurst() << "]"); 
